...
class AdminAnalyticsUseCase {
    ...
    async getOverview() {
        ...
        const [totalUsers, newUsersThisMonth, activePartners, pendingPartnerRequests, activeMovies, pendingMovies, revenueThisMonth, revenuePrevMonth, ticketsSoldThisMonth, pendingWithdrawals,] = await Promise.all([
    ...
    async getUserTrend(period) {
        ...
        for (const u of users) {
            ...
            if (!buckets[key])
                buckets[key] = { total: 0, users: 0, partners: 0 };
            buckets[key].total += 1;
            if (u.role === "USER")
                buckets[key].users += 1;
            if (u.role === "PARTNER")
                buckets[key].partners += 1;
        }
        return {
            period,
            groupBy,
