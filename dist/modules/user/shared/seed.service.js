    ...
    async seedUsers(options, progressCallback) {
        ...
        try {
            ...
            for (let batch = 0; batch < totalBatches; batch++) {
                ...
                try {
                    ...
                    try {
                        ...
                        const errorMsg = `Batch ${batch + 1} failed: ${batchError instanceof Error
                            ? batchError.message
                            : String(batchError)}`;
            ...
            try {
                ...
                const errorMsg = `Error creating user settings: ${error instanceof Error ? error.message : String(error)}`;
                errors.push(errorMsg);
                progressCallback?.onError?.(errorMsg);
            }
        }
