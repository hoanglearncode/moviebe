...
exports.createPaymentRepository = exports.PaymentRepository = void 0;
...
class PaymentRepository {
    ...
    async createPaymentForOrder(userId, orderId, paymentMethod) {
        const order = await this.prisma.order.findUnique({ where: { id: orderId } });
        if (!order)
            throw new error_1.PaymentOrderNotFoundError();
        if (order.userId !== userId)
            throw new error_1.PaymentAccessDeniedError();
        if (order.status !== "PENDING")
            throw new error_1.OrderNotPendingError(order.status);
        const now = new Date();
        // Generate deterministic-ish gateway ref for this order
        const gatewayRef = `MOCK-${orderId.slice(0, 8).toUpperCase()}-${(0, crypto_1.randomUUID)().slice(0, 8).toUpperCase()}`;
        ...
        const frontendUrl = value_1.ENV.FRONTEND_URL.replace(/\/$/, "");
...
const createPaymentRepository = (prisma) => new PaymentRepository(prisma);
