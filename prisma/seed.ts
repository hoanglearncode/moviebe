...
const addHours = (base: Date, h: number) => new Date(base.getTime() + h * 3_600_000);
...
async function main() {
  ...
  await prisma.user.createMany({
    data: [
      ...
      ...regularUserIds.map((id, i) => ({
        ...
        password: seedPass,
        role: Role.USER,
        status: i < 25 ? UserStatus.ACTIVE : UserStatus.INACTIVE,
        emailVerified: i < 28,
        provider: 'local',
        phone: `09${r(10_000_000, 99_999_999)}`,
  ...
  await prisma.partnerWallet.createMany({
    data: partnerIds.map((partnerId) => ({
      id: uid(),
      partnerId,
      balance: r(5_000_000, 50_000_000),
      totalEarned: r(100_000_000, 500_000_000),
      totalWithdrawn: r(20_000_000, 100_000_000),
      totalRefunded: r(0, 5_000_000),
    })),
    skipDuplicates: true,
  });
  ...
  await prisma.category.createMany({
    data: [
      { id: 'cat_action', name: 'Hành Động', slug: 'hanh-dong', description: 'Phim hành động, võ thuật, phiêu lưu', status: ModelStatus.active, position: 1 },
      { id: 'cat_comedy', name: 'Hài Hước', slug: 'hai-huoc', description: 'Phim hài, giải trí', status: ModelStatus.active, position: 2 },
      { id: 'cat_drama', name: 'Tâm Lý', slug: 'tam-ly', description: 'Phim tâm lý, chính kịch', status: ModelStatus.active, position: 3 },
      { id: 'cat_horror', name: 'Kinh Dị', slug: 'kinh-di', description: 'Phim kinh dị, ma quái', status: ModelStatus.active, position: 4 },
      { id: 'cat_scifi', name: 'Khoa Học Viễn Tưởng', slug: 'khoa-hoc-vien-tuong', description: 'Phim sci-fi, tương lai', status: ModelStatus.active, position: 5 },
      { id: 'cat_romance', name: 'Tình Cảm', slug: 'tinh-cam', description: 'Phim tình cảm, lãng mạn', status: ModelStatus.active, position: 6 },
      { id: 'cat_animation', name: 'Hoạt Hình', slug: 'hoat-hinh', description: 'Phim hoạt hình, animation', status: ModelStatus.active, position: 7 },
      { id: 'cat_thriller', name: 'Hồi Hộp', slug: 'hoi-hop', description: 'Phim hồi hộp, ly kỳ', status: ModelStatus.active, position: 8 },
    ],
    skipDuplicates: true,
  });
  ...
  await prisma.room.createMany({
    data: partnerIds.flatMap((partnerId, pi) =>
      roomConfigs.map((cfg, ri) => ({
        id: roomIds[pi * 3 + ri],
        partnerId,
        name: cfg.name,
        type: cfg.type,
  ...
  const serviceDefs = [
    { name: 'Bỏng ngô vừa', price: 35_000, category: 'food', icon: '🍿', description: 'Bỏng ngô bơ thơm ngon' },
    { name: 'Nước ngọt lớn', price: 28_000, category: 'drinks', icon: '🥤', description: 'Coca-Cola / Pepsi size L' },
    { name: 'Combo đôi', price: 75_000, category: 'combo', icon: '🎁', description: '2 bỏng vừa + 2 nước lớn' },
  ];

  const createdServiceIds: { id: number; partnerIdx: number }[] = [];
  ...
  await prisma.cast.createMany({
    data: CAST_DATA.flatMap((casts, mi) =>
      casts.map((c) => ({ id: uid(), movieId: movieIds[mi], ...c }))
    ),
  });
      ...
      data: [
        {
          ...
          basePrice: 80_000,
          priceConfig: { STANDARD: 80_000, VIP: 120_000 },
          status: ShowtimeStatus.ENDED,
        ...
        },
        {
          ...
          basePrice: 95_000,
          priceConfig: { STANDARD: 95_000, VIP: 145_000 },
  ...
  type SeatInfo = { id: string; seatNumber: string; price: number; seatType: SeatType };
  const seatMap = new Map<string, SeatInfo[]>();

  const allSeats: {
    id: string; showtimeId: string; seatNumber: string; rowLabel: string;
    columnNumber: number; seatType: SeatType; status: SeatStatus; price: number;
  }[] = [];

  for (const stId of showtimeIds) {
    const stSeats: SeatInfo[] = [];
    for (const row of ['A', 'B']) {
      for (let col = 1; col <= 5; col++) {
        const seatId = uid();
        const seatType = row === 'B' ? SeatType.VIP : SeatType.STANDARD;
        const price = seatType === SeatType.VIP ? 120_000 : 80_000;
        allSeats.push({ id: seatId, showtimeId: stId, seatNumber: `${row}${col}`, rowLabel: row, columnNumber: col, seatType, status: SeatStatus.AVAILABLE, price });
  ...
  const createdTickets: { id: string; partnerId: string; showtimeId: string; userId: string; status: TicketStatus }[] = [];
  ...
  for (let oi = 0; oi < 15; oi++) {
    ...
    for (const seat of pickedSeats) {
      ...
      const ticketStatus = isUsed ? TicketStatus.USED : TicketStatus.CONFIRMED;
      ...
      await prisma.seat.update({ where: { id: seat.id }, data: { status: SeatStatus.BOOKED } });
  ...
  const usedTickets = createdTickets.filter((t) => t.status === TicketStatus.USED);
  ...
  await prisma.review.createMany({
    data: Array.from({ length: 20 }, (_, i) => ({
      ...
      status: i < 16 ? ReviewStatus.APPROVED : ReviewStatus.PENDING,
  ...
  const wdStatuses = [WithdrawalStatus.COMPLETED, WithdrawalStatus.PENDING, WithdrawalStatus.PROCESSING];
  await prisma.withdrawal.createMany({
    data: [0, 1, 2].map((i) => ({
      id: uid(),
      partnerId: partnerIds[i],
      amount: r(10_000_000, 50_000_000),
  ...
  const partnerNotifs = [
    { userId: partnerUserIds[0], type: NotificationType.PARTNER_MOVIE_APPROVED, title: 'Phim được duyệt', message: 'Phim "Lật Mặt 7" đã được admin phê duyệt và sẵn sàng bán vé.' },
    { userId: partnerUserIds[1], type: NotificationType.PARTNER_MOVIE_APPROVED, title: 'Phim được duyệt', message: 'Phim "Mai" đã được admin phê duyệt và sẵn sàng bán vé.' },
    { userId: partnerUserIds[0], type: NotificationType.PARTNER_WITHDRAWAL_COMPLETED, title: 'Rút tiền thành công', message: 'Yêu cầu rút tiền 20.000.000 VND đã hoàn tất. Kiểm tra tài khoản ngân hàng của bạn.' },
    { userId: partnerUserIds[1], type: NotificationType.PARTNER_WITHDRAWAL_PENDING, title: 'Yêu cầu rút tiền đang xử lý', message: 'Yêu cầu rút tiền đang được xử lý, vui lòng chờ 1-3 ngày làm việc.' },
    { userId: partnerUserIds[2], type: NotificationType.PARTNER_DAILY_REVENUE, title: 'Doanh thu hôm nay', message: 'Doanh thu ngày hôm nay: 5.250.000 VND từ 70 vé bán ra.' },
    { userId: partnerUserIds[3], type: NotificationType.PARTNER_MOVIE_REJECTED, title: 'Phim bị từ chối', message: 'Phim của bạn bị từ chối do thiếu thông tin. Vui lòng cập nhật và gửi lại.' },
  ];

  const userNotifs = regularUserIds.slice(0, 12).map((userId, i) => ({
    userId,
    type: i % 3 === 0 ? NotificationType.BOOKING_CONFIRMED : i % 3 === 1 ? NotificationType.BOOKING_CANCELLED : NotificationType.TICKET_REFUNDED,
  ...
  await prisma.partnerStaff.createMany({
    data: [
      { id: uid(), partnerId: partnerIds[0], userId: regularUserIds[20], role: StaffRole.MANAGER },
      { id: uid(), partnerId: partnerIds[0], userId: regularUserIds[21], role: StaffRole.CASHIER },
      { id: uid(), partnerId: partnerIds[1], userId: regularUserIds[22], role: StaffRole.SCANNER },
      { id: uid(), partnerId: partnerIds[2], userId: regularUserIds[23], role: StaffRole.STAFF },
      { id: uid(), partnerId: partnerIds[3], userId: regularUserIds[24], role: StaffRole.MANAGER },
    ],
    skipDuplicates: true,
  });
