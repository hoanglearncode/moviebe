...
exports.AdminFinanceUseCase = void 0;
const client_1 = require("@prisma/client");
class AdminFinanceUseCase {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async getSummary(period = "12m") {
        const startDate = this.periodToStart(period);
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        const startOfPrevMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
        const [totalRevenue, totalPayout, revenueThisMonth, revenuePrevMonth, payoutThisMonth, pendingWithdrawals, processingWithdrawals, completedWithdrawals,] = await Promise.all([
    ...
    async completeWithdrawal(withdrawalId, transactionReference) {
        const withdrawal = await this.prisma.withdrawal.findUnique({
            where: { id: withdrawalId },
        });
        if (!withdrawal)
...
exports.AdminFinanceUseCase = AdminFinanceUseCase;
