...
exports.MovieManagementUseCase = void 0;
const crypto_1 = require("crypto");
const model_1 = require("../model/model");
class MovieManagementUseCase {
    constructor(partnerRepo, movieRepo, showtimeRepo, seatRepo, roomRepo) {
    ...
    validateDateRange(releaseDate, endDate) {
        if (releaseDate >= endDate)
            throw new Error("Release date must be before end date");
    }
    async createMovie(partnerId, data) {
        ...
        const movieId = (0, crypto_1.randomUUID)();
        const now = new Date();
        const movie = {
            id: movieId,
            partnerId,
     ...
     */
    async _createShowtimesFromPlans(movieId, partnerId, durationMinutes, plans) {
        const ROWS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        const SEAT_TYPE_MAP = {
            0: model_1.SeatType.STANDARD,
            1: model_1.SeatType.VIP,
            2: model_1.SeatType.COUPLE,
        };
        for (const plan of plans) {
            ...
            const layout = room.layoutSeat ?? [];
            const priceConfig = {};
            if (plan.prices.standard)
                priceConfig.standard = plan.prices.standard;
            if (plan.prices.vip)
                priceConfig.vip = plan.prices.vip;
            if (plan.prices.premium)
                priceConfig.premium = plan.prices.premium;
            if (plan.prices.couple)
                priceConfig.couple = plan.prices.couple;
            ...
            for (let r = 0; r < layout.length; r++) {
                ...
                for (let c = 0; c < row.length; c++) {
                    ...
                    const colNumber = c + 1;
                    const seatType = SEAT_TYPE_MAP[cellValue] ?? model_1.SeatType.STANDARD;
                    const seatTypeKey = seatType.toLowerCase();
    ...
    async deleteMovie(partnerId, movieId) {
        ...
        if (movie.status !== "DRAFT")
            throw new Error("Only DRAFT movies can be deleted");
    ...
    async adminRejectMovie(movieId, reason, note) {
        const movie = await this.movieRepo.findById(movieId);
        if (!movie)
            throw new Error("Movie not found");
        if (movie.status !== "SUBMITTED")
            throw new Error("Only SUBMITTED movies can be rejected");
